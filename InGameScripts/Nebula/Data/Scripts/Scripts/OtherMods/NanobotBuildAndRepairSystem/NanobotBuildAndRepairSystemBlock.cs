namespace SpaceEquipmentLtd.NanobotBuildAndRepairSystem
{
   ...
   public class NanobotBuildAndRepairSystemBlock : MyGameLogicComponent {
      ...
      public const int WELDER_RANGE_DEFAULT_IN_M = 75; //*2 = AreaSize
      ...
      public const float WELDER_REQUIRED_ELECTRIC_POWER_STANDBY_DEFAULT = 100f; //20W
      public const float WELDER_REQUIRED_ELECTRIC_POWER_WELDING_DEFAULT = 50f; //2kW
      public const float WELDER_REQUIRED_ELECTRIC_POWER_GRINDING_DEFAULT = 50f; //1.5kW
      public const float WELDER_REQUIRED_ELECTRIC_POWER_TRANSPORT_DEFAULT = 0.5f; //500kW
      public const float WELDER_TRANSPORTSPEED_METER_PER_SECOND_DEFAULT = 100f;
      public const float WELDER_TRANSPORTVOLUME_DIVISOR = 4f;
      public const float WELDER_TRANSPORTVOLUME_MAX_MULTIPLIER = 40f;
      public const float WELDER_AMOUNT_PER_SECOND = 120f;
      public const float WELDER_MAX_REPAIR_BONE_MOVEMENT_SPEED = 1f;
      public const float GRINDER_AMOUNT_PER_SECOND = 7f;
      ...
      public void SettingsChanged()
      {
         ...
         _MaxTransportVolume = ((float)_TransportInventory.MaxVolume) / WELDER_TRANSPORTVOLUME_DIVISOR;
      ...
      private void Init()
      {
         ...
         if (Mod.Log.ShouldLog(Logging.Level.Event)) Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: Init Block TransportInventory MaxVolume {1}", Logging.BlockName(_Welder, Logging.BlockNameOptions.None), (float)welderInventory.MaxVolume / MyAPIGateway.Session.BlocksInventorySizeMultiplier);
         SettingsChanged();
      ...
      private void UpdateBeforeSimulation10_100(bool fast) {
         try {
            ...
            if (!_IsInit) Init();
            if (!_IsInit) return;
            //if (!_Welder.CubeGrid.IsStatic) return;
            ...
            if (MyAPIGateway.Session.IsServer) {
               ...
               if (!fast) {
                  if ((State.Ready != _PowerReady || State.Welding != _PowerWelding || State.Grinding != _PowerGrinding || State.Transporting != _PowerTransporting) && MyAPIGateway.Session.ElapsedPlayTime.Subtract(_UpdatePowerSinkLast).TotalSeconds > 5) {
