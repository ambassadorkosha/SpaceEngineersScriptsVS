...
namespace ScriptKROTFix
{
    public sealed class Program : MyGridProgram
    {
        //------------BEGIN--------------

        const float max_speed_flight = 19.0f; //макс скорость полета
        const float max_speed_drill = 3f; //макс скорость проходки
        const float max_hover_h = 22.0f; //макс высота ховера
        ...
        public Program()
        {
            ...
            GridTerminalSystem.GetBlocksOfType<IMyCockpit>(TempList, (b) => (b.IsSameConstructAs(Me) && b.CustomName.Contains("Industrial Cockpit [Kopalko I]")));
            if (TempList.Count > 0)
            {
