...
namespace Octopus
{
    public sealed class Program : MyGridProgram
    {
        ...
        int TickCount;
        int Clock = 10;
        string ShipName = "Octopus (5)";
        string NewName = "Octopus(5)";
        float GyroMult = 7f;
        ...
        public class MyDriller
        {
            ...
            private class MyThrusters
            {
                ...
                public void SetOverridePercent(string axis, float OverrideValue)
                {
                    ...
                    for (int i = 0; i < Thrusts.Count; i++)
                    {
                        ...
                        if (Thrust != null)
                        {
                            ...
                            Thrust.ThrustOverridePercentage = OverrideValue / 100;
                        }
                    }
                }
                ...
                public void SetOverrideN(string axis, float OverrideValue)
                {
                    ...
                    if (axis == "U") { MaxThrust = UMaxT; SetOverridePercent("D", 0f); }
                    else if (axis == "D") { MaxThrust = DMaxT; SetOverridePercent("U", 0f); }
                    else if (axis == "F") { MaxThrust = FMaxT; SetOverridePercent("B", 0f); }
                    else if (axis == "B") { MaxThrust = BMaxT; SetOverridePercent("F", 0f); }
                    else if (axis == "R") { MaxThrust = RMaxT; SetOverridePercent("L", 0f); }
                    else if (axis == "L") { MaxThrust = LMaxT; SetOverridePercent("R", 0f); }
                    ...
                    for (int i = 0; i < Thrusts.Count; i++)
                    {
                        ...
                        if (Thrust != null)
                        {
                            ...
                            else
                            {
                                ...
                                Thrust.ThrustOverridePercentage = Math.Max(OverrideValue / MaxThrust, 0.1f);
                            }
                        }
                    }
                }
            ...
            private class MyNavigation
            {
                ...
                public double GetVal(string Key, IMyTextPanel ScrObj)
                {
                    string val = "0";
                    string pattern = @"(" + Key + "):([^:^;]+);";
                    System.Text.RegularExpressions.Match match = System.Text.RegularExpressions.Regex.Match(ScrObj.GetText().Replace("\n", ""), pattern);
                    if (match.Success)
                    {
                ...
                public Vector3D GetRockByNum(int Key, IMyTextPanel ScrObj)
                {
                    ...
                    System.Text.RegularExpressions.Match match = System.Text.RegularExpressions.Regex.Match(ScrObj.GetText(), pattern);
                    if (match.Success)
                    {
                ...
                public void StartRockScavenging()
                {
                    ...
                    string RockStr = ScrObj.GetText();
                    string[] strs = RockStr.Split('\n');
                    MaxRocks = strs.Length - 1; //GetValInt("Rocks found", ScrObj);
                    SetRock(RockN);
                    SaveToStorage();
                }
