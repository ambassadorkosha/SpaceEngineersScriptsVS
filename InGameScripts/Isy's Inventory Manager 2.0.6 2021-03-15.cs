...
ContentType.TEXT_AND_IMAGE;
			...
			foreach (var J in ʟ)
			{
				...
				foreach (var Ð in Ò)
				{
					if (!Ð.StartsWith("-")) continue; 
					string ϊ = ""; try
					{
						if (Ð.Contains("=")) 
						{
							ϊ = Ð.Substring(1, Ð.IndexOf("=") - 1);
						}
						else
						{
							ϊ = Ð.Substring(1);
				...
				if (!ϋ)
				{
					MyDefinitionId M = ǟ(J);
					double ˢ = Math.Ceiling(G(M)); 
					κ.Add(J + " " + ˢ + " = " + ˢ + defaultModifier);
				}
			}
			foreach (var Ð in Ò)
			{
			...
			StringBuilder Ǝ = new StringBuilder();
			try
			{
				IOrderedEnumerable<string> ω;
				ω = κ.OrderBy(ɑ => ɑ);
				bool ψ; 
				string χ, J, φ;
				foreach (var Ð in ω)
				{
					...
					if (Ð.StartsWith("-")) 
					{
						J = Ð.Remove(Ð.IndexOf("=")).TrimStart('-');
						χ = "-";
					}
					else 
					{
						J = Ð.Remove(Ð.IndexOf(" "));
						χ = "";
					}
					φ = Ð.Replace(χ + J, "");
					foreach (var Ë in ʟ) 
		...
		void ν(StringBuilder ń)
		{
			...
			foreach (var u in ʍ)
			{
				...
				int Ĺ = u.Ŏ(); 
				int ĸ = 0;
				List<string> Ǝ = new List<string>();
				if (u == ʍ[0] || headerOnEveryScreen)
				{
					string ί = ɕ; if (headerOnEveryScreen && ʍ.Count > 1)
					{
						ί += " " + (ʍ.IndexOf(u) + 1) + "/" + ʍ.Count;
						try 
						{
							ί += " [" + Ľ[ĺ][0] + "-#]"; 
						}
					...
					Ǝ.Add(ί); 
					Ǝ.Add(u.ō('=', u.ũ(ί)).ToString() + "\n"); 
					string ή = "Component "; 
					string έ = "Current | Wanted "; ΰ = u.ũ("Wanted ");
					string Ǥ = u.ō(' ', ų - u.ũ(ή) - u.ũ(έ)).ToString();
					Ǝ.Add(ή + Ǥ + έ + "\n");
					ĸ = 5;
				} while ((ĺ < Ļ && ĸ < Ĺ) || (u == ʍ[ʍ.Count - 1] && ĺ < Ļ))
				{
					...
					Ð[0] += " ";
					Ð[1] = Ð[1].Replace('$', ' ');
					string Ǥ = u.ō(' ', ų - u.ũ(Ð[0]) - u.ũ(Ð[1]) - ΰ).ToString();
					string ά = Ð[0] + Ǥ + Ð[1] + Ð[2];
					Ǝ.Add(ά); 
					ĺ++;
					ĸ++;
				}
				if (headerOnEveryScreen && ʍ.Count > 1) 
				{
					Ǝ[0] = Ǝ[0].Replace('#', Ľ[ĺ - 1][0]); 
				}
		...
		void μ()
		{
			...
			if (ʁ.Count == 0)
			{
				ƹ("No assemblers found!\n" +
					"Build assemblers to enable autocrafting!");
				return;
			}
			ˮ(); 
			List<MyDefinitionId> λ = new List<MyDefinitionId>(); 
			var κ = ρ().ToString().TrimEnd('\n').Split('\n');
			StringBuilder Ǝ = new StringBuilder();
			foreach (var Ð in κ)
			{
				string J = ""; 
				...
				MyDefinitionId M = ǟ(J);
				if (M == null) continue;
				double θ = Math.Ceiling(G(M)); 
				string η = Ð.Substring(Ð.IndexOfAny(ȗ) + 1).ToLower();
				double ζ = 0; 
				double.TryParse(System.Text.RegularExpressions.Regex.Replace(η, @"\D", ""), out ζ); 
				string ε = θ.ToString(); 
				string δ = ζ.ToString();
				string γ = ""; 
				bool æ = false;
				if (η.Contains("h") && ι)
				{
					if (!ʍ[0].CustomData.StartsWith(Ȼ)) ʍ[0].CustomData = Ȼ;
					ʍ[0].CustomData += "\n-" + J + "=" + η.Replace("h", "").Replace(" ", "").ToUpper();
					continue;
				}
				...
				Ɲ(M, ζ);
				double β = Math.Abs(ζ - θ); 
				bool Έ; 
				MyDefinitionId Ô = Ʀ(M, out Έ);
				double ˏ = ƞ(Ô); 
				if (θ >= ζ + ζ * assembleMargin && ˏ > 0 && ƪ(Ô) > 0)
				{
					ˍ(Ô);
					Ʃ(Ô, 0);
					ˏ = 0;
					ɞ = "Removed '" + M.SubtypeId.ToString() + "' from the assembling queue.";
				}
				if (θ <= ζ - ζ * disassembleMargin && ˏ > 0 && ƪ(Ô) < 0)
				{
					...
					Ʃ(Ô, 0); 
					ˏ = 0;
					ɞ = "Removed '" + M.SubtypeId.ToString() + "' from the disassembling queue.";
				}
				string Ù = ""; if (ˏ > 0 || β > 0)
				{
					if ((enableAutodisassembling || η.Contains("d")) && θ > ζ + ζ * disassembleMargin) 
					{
						Ʃ(Ô, -1);
						Ù = "$[D:";
					}
				...
				if (θ > ζ) ʻ = "$>$ "; 
				if (θ < ζ) ʻ = "$<$ "; 
				if (ι) Ǝ.Append(J + " " + ε + Ù + ʻ + δ + γ + "\n");
				if (Ù.Contains("[D:Wait]")) 
				{
					ˎ(Ô, β); 
				}
				else if (Ù.Contains("[A:Wait]"))
				{
					Ά(Ô, β, æ);
					ɞ = "Queued " + β + " '" + M.SubtypeId.ToString() + "' in the assemblers."; 
				}
				else if (Ù.Contains("[NoBP!]") && ζ > θ)
				{
					ƹ("Can't craft\n'" + 
						M.SubtypeId.ToString() +
						"'\nThere's no blueprint stored for this item!\n" +
						"Tag an assembler with the '" + 
						learnKeyword + 
						"' keyword and queue\n" +
		...
		void ʷ()
		{
			if (ɧ == 0) ɧ += ʶ(ʛ, Ȏ, true, true);
			if (ɧ == 1) ɧ += ʶ(ʚ, ȍ, true, true); 
			if (ɧ == 2) ɧ += ʶ(ʙ, Ȍ, true, true); 
			if (ɧ == 3) ɧ += ʶ(ʘ, ȉ, true, true); 
			if (ɧ == 4) ɧ += ʶ(ʗ, ȋ, true, true);
			if (ɧ == 5) ɧ += ʶ(ʖ, "ContainerObject", true, true);
			ɧ++;
			if (ɧ > 5) ɧ = 0;
		}
		int ʶ(List<IMyTerminalBlock> ɒ, string ʵ = "", bool ʴ = false, bool ʳ = false)
		{
			if (ʴ) ɒ.RemoveAll(Ū => Ū.InventoryCount == 2 || Ū.BlockDefinition.TypeIdString.Contains("OxygenGenerator") || Ū.BlockDefinition.TypeIdString.Contains("OxygenTank"));
			if (ʳ) ɒ.RemoveAll(H => !H.CubeGrid.IsSameConstructAs(Me.CubeGrid));
			if (ɒ.Count < 2) { return 1; }
			Dictionary<MyItemType, double> ʽ = new Dictionary<MyItemType, double>(); 
			for (int H = 0; H < ɒ.Count; H++)
			{
				var A = new List<MyInventoryItem>(); ɒ[H].GetInventory(0).GetItems(A); 
				foreach (var Ë in A)
				{
					if (!Ë.Type.TypeId.ToString().Contains(ʵ)) continue; 
					MyItemType M = Ë.Type; 
					if (ʽ.ContainsKey(M))
					{
						ʽ[M] += (double)Ë.Amount;
			...
			Dictionary<MyItemType, double> ˈ = new Dictionary<MyItemType, double>(); foreach (var Ë in ʽ) { ˈ[Ë.Key] = (int)(Ë.Value / ɒ.Count); }
			for (int ˇ = 0; ˇ < ɒ.Count; ˇ++)
			{
				if (Ǔ()) return 0;
				var ˆ = new List<MyInventoryItem>(); 
				ɒ[ˇ].GetInventory(0).GetItems(ˆ); 
				Dictionary<MyItemType, double> ˁ = new Dictionary<MyItemType, double>();
				foreach (var Ë in ˆ)
				{
					MyItemType M = Ë.Type; 
					if (ˁ.ContainsKey(M))
				...
				{
					ˁ.TryGetValue(Ë.Key, out Ģ); 
					double ˀ = ˈ[Ë.Key];
					if (Ģ <= ˀ + 1) continue;
					for (int ʿ = 0; ʿ < ɒ.Count; ʿ++)
					{
						if (ɒ[ˇ] == ɒ[ʿ]) continue;
						double Ğ = G(Ë.Key, ɒ[ʿ]);
						if (Ğ >= ˀ - 1) continue; 
						double ʾ = ˀ - Ğ; if (ʾ > Ģ - ˀ) ʾ = Ģ - ˀ; 
						if (ʾ > 0) 
						{
							Ģ -= Æ(Ë.Key.ToString(), ɒ[ˇ], 0, ɒ[ʿ], 0, ʾ, true); 
							if (Ģ.ƚ(ˀ - 1, ˀ + 1)) break;
						}
					}
				}
			}
		...
		{
			if (ɟ == 0)
				ɟ += ʶ(ʄ.ToList<IMyTerminalBlock>()); 
			if (ɟ == 1) ɟ += ʶ(ʂ.ToList<IMyTerminalBlock>());
			ɟ++; 
			if (ɟ > 1) ɟ = 0;
		}
		void ʩ(List<IMyRefinery> ʨ, List<MyItemType> ʡ)
		{
			foreach (IMyRefinery Ý in ʨ)
			{
				var Q = Ý.GetInventory(0);
				...
				foreach (var ʤ in ʡ)
				{
					for (int H = 0; H < A.Count; H++)
					{
						if (A[H].Type == ʤ)
						{
							ʧ = H; ʥ = ʤ.SubtypeId; ʪ = true; break;
						}
					}
					if (ʪ) break;
				}
				if (ʧ != 0)
				{
					Q.TransferItemTo(Q, ʧ, 0, true); 
					ɞ = "Sorted the refining queue.\n'" + ʥ + "' is now at the front of the queue.";
				}
			}
		}
		void ʣ(List<IMyRefinery> ʢ, List<MyItemType> ʡ)
		{
			...
			MyItemType ʦ = new MyItemType(); 
			MyItemType ʬ = MyItemType.MakeOre("Stone");
			foreach (var ʤ in ʡ)
			{
				if (G(ʤ) > 100) 
				{
			...
			if (!ʦ.ToString().Contains(Ȏ)) return;
			for (int H = 0; H < ʢ.Count; H++)
			{
				if (Ǔ()) return; 
				var Q = ʢ[H].GetInventory(0);
				if ((float)Q.CurrentVolume > (float)Q.MaxVolume * 0.75f)
				{
					var A = new List<MyInventoryItem>(); 
					Q.GetItems(A); 
					foreach (var Ë in A) 
		...
		void ʱ(List<IMyRefinery> ʢ, List<MyItemType> ʡ)
		{
			...
			{
				if (G(ʤ) == 0) continue; 
				IMyTerminalBlock ʯ = d(ʤ, true);
				if (ʯ == null) continue; 
				for (int H = 0; H < ʢ.Count; H++)
				{
					if (Ǔ()) return;
					var Q = ʢ[H].GetInventory(0);
					if ((float)Q.CurrentVolume > (float)Q.MaxVolume * 0.98f) continue;
					ʰ = Æ(ʤ.ToString(), ʯ, 0, ʢ[H], 0); 
					if (ʰ == 0)
					{
						ʯ = d(ʤ, true);
						if (ʯ == null) break;
		...
		bool ʮ(List<IMyRefinery> ʭ, List<IMyRefinery> ˉ, List<MyItemType> ʡ)
		{
			...
			{
				if ((float)ʭ[H].GetInventory(0).CurrentVolume > 0.05f) continue;
				for (int ƺ = 0; ƺ < ˉ.Count; ƺ++)
				{
					if ((float)ˉ[ƺ].GetInventory(0).CurrentVolume > 0) 
					{
						foreach (var ʤ in ʡ) 
						{
							Æ(ʤ.ToString(), ˉ[ƺ], 0, ʭ[H], 0, -0.5); 
						}
						return true; 
					}
				}
		...
		List<MyItemType> ͳ(List<IMyRefinery> ʢ)
		{
			if (ʢ.Count == 0) 
			{
				ɡ++; 
			...
			List<string> Ͳ = new List<string>(ȓ); 
			Ͳ.Sort((ɑ, ŧ) => (G(MyItemType.MakeIngot(ɑ)) / ơ(ɑ)).CompareTo((G(MyItemType.MakeIngot(ŧ)) / ơ(ŧ))));
			Ͳ.InsertRange(0, fixedRefiningList); 
			List<MyItemType> ʹ = new List<MyItemType>();
			MyItemType M;
			foreach (var Ë in Ͳ)
			{
				M = MyItemType.MakeOre(Ë); 
				foreach (var Ý in ʢ)
				{
		...
		void Ͱ()
		{
			foreach (var Ø in ʁ)
			{
				...
				{
					var Q = Ø.GetInventory(0);
					if ((float)Q.CurrentVolume == 0) continue;
					if (Ø.IsQueueEmpty || Ø.Mode == MyAssemblerMode.Disassembly || (float)Q.CurrentVolume > (float)Q.MaxVolume * 0.98f)
					{
						IMyTerminalBlock Y = e(Ø, ʚ); 
						if (Y != null) Æ("", Ø, 0, Y, 0); 
					}
				}
			}
		...
		void ͱ()
		{
			...
			{
				if (Ø.Mode == MyAssemblerMode.Disassembly) continue;
				if (Ø.CustomData.Contains("skipQueueSorting")) 
				{
					Ø.CustomData = "";
					continue; 
				}
				var Ù = new List<MyProductionItem>();
				Ø.GetQueue(Ù); 
				if (Ù.Count < 2) continue;
				int ʧ = 0; 
				string ʥ = "";
				double ͽ = Double.MaxValue;
				double ͼ = Double.MinValue;
				double ͻ, ͺ;
				for (int H = 0; H < Ù.Count; H++)
				{
					MyDefinitionId M = Ƥ(Ù[H].BlueprintId);
					ͺ = G(M); 
					ͻ = (double)Ù[H].Amount;
					if (ͺ < 100 && ͺ < ͽ) 
					{
						ͽ = ͺ; 
						ʧ = H;
						ʥ = M.SubtypeId.ToString();
						continue;
					}
					if (ͽ == Double.MaxValue && ͻ > ͼ)
					{
						ͼ = ͻ; 
						ʧ = H; 
						ʥ = M.SubtypeId.ToString(); 
				...
				if (ʧ != 0)
				{
					Ø.MoveQueueItemRequest(Ù[ʧ].ItemId, 0);
					ɞ = "Sorted the assembling queue.\n'" + ʥ + "' is now at the front of the queue.";
				}
			}
		}
		...
		{
			if (Ͷ.Count == 0) return; 
			if (Ͷ.Count > 1) Ͷ.Sort((ɑ, ŧ) => G(Ƥ(ɑ)).CompareTo(G(Ƥ(ŧ))));
			foreach (var Ø in ʁ)
			{
				var Ù = new List<MyProductionItem>(); 
				Ø.GetQueue(Ù);
				...
				foreach (var Ô in Ͷ)
				{
					int ƙ = Ù.FindIndex(H => H.BlueprintId == Ô); 
					if (ƙ == -1) continue;
					if (ƙ == 0) 
					{
						Ø.CustomData = "skipQueueSorting";
						break; 
					}
					Ø.MoveQueueItemRequest(Ù[ƙ].ItemId, 0);
					Ø.CustomData = "skipQueueSorting";
					ɞ = "Sorted the assembler queue by priority.\n'" +
		...
		{
			List<IMyAssembler> ˋ = new List<IMyAssembler>(); 
			foreach (IMyAssembler Ø in ʁ)
			{
				if (Ø.CustomName.Contains(disassembleKeyword)) continue;
				if (æ == false && Ø.Mode == MyAssemblerMode.Disassembly && !Ø.IsQueueEmpty) continue; 
				if (Ø.Mode == MyAssemblerMode.Disassembly) 
				{
					Ø.ClearQueue(); 
					Ø.Mode = MyAssemblerMode.Assembly; 
				}
			...
			if (ˋ.Count == 0) ƹ("There's no assembler available to produce '" + 
				Ô.SubtypeName +
		...
		void ˎ(MyDefinitionId Ô, double Á)
		{
			...
			foreach (IMyAssembler Ø in ʁ)
			{
				if (Ø.CustomName.Contains(assembleKeyword)) continue;
				if (Ø.Mode == MyAssemblerMode.Assembly && Ø.IsProducing) continue; 
				if (Ø.Mode == MyAssemblerMode.Assembly) 
				{
					Ø.ClearQueue();
					Ø.Mode = MyAssemblerMode.Disassembly; 
				}
				if (Ø.Mode == MyAssemblerMode.Assembly) continue;
				if (Ø.CanUseBlueprint(Ô)) 
				{
					ˋ.Add(Ø);
				}
			}
			if (ˋ.Count == 0) ƹ("There's no assembler available to dismantle '" + 
				Ô.SubtypeName +
		...
		{
			if (ˋ.Count == 0) return;
			double ˊ = Math.Ceiling(Á / ˋ.Count);
			foreach (IMyAssembler Ø in ˋ) 
			{
				if (ˊ > Á) ˊ = Math.Ceiling(Á); 
				if (Á > 0)
				{
					Ø.InsertQueueItem(0, Ô, ˊ);
					Á -= ˊ; 
				}
				else
				{
		...
		void ˍ(MyDefinitionId Ô)
		{
			foreach (IMyAssembler Ø in ʁ)
			{
				...
				for (int H = 0; H < Ù.Count; H++)
				{
					if (Ù[H].BlueprintId == Ô) Ø.RemoveQueueItem(H, Ù[H].Amount);
				}
			}
		}
		void ˮ()
		{
			foreach (IMyAssembler Ø in ʁ)
			{
				Ø.UseConveyorSystem = true;
				Ø.CooperativeMode = false;
				Ø.Repeating = false;
			}
		}
		void ˬ()
		{
			List<IMyAssembler> ˤ = new List<IMyAssembler>(ʁ);
			ˤ.RemoveAll(ɑ => ɑ.IsQueueEmpty); 
			if (ˤ.Count == 0) return;
			List<IMyAssembler> ˣ = new List<IMyAssembler>(ʁ);
			ˣ.RemoveAll(ɑ => !ɑ.IsQueueEmpty);
			if (ˣ.Count == 0) return; 
			double ˢ, ˡ = 0, ˠ; 
			IMyAssembler ˑ = null; 
			List<MyProductionItem> Ù = new List<MyProductionItem>();
			foreach (var ː in ˤ)
			{
				Ù.Clear();
				ː.GetQueue(Ù); 
			...
			for (int H = 0; H < Ù.Count; H++)
			{
				ˢ = (double)Ù[H].Amount; if (ˢ <= 10) continue;
				for (int ƺ = 0; ƺ < ˣ.Count; ƺ++)
				{
					if (!ˣ[ƺ].CanUseBlueprint(Ù[0].BlueprintId)) continue; 
					if (ˑ.Mode == MyAssemblerMode.Assembly && ˣ[ƺ].CustomName.Contains(disassembleKeyword)) continue; 
					if (ˑ.Mode == MyAssemblerMode.Disassembly && ˣ[ƺ].CustomName.Contains(assembleKeyword)) continue;
					ˣ[ƺ].Mode = ˑ.Mode; 
					if (ˣ[ƺ].Mode != ˑ.Mode) continue;
					ˠ = Math.Ceiling(ˢ / ˣ.Count + 1 - ƺ); 
					ˢ -= ˠ;
					ˣ[ƺ].AddQueueItem(Ù[H].BlueprintId, ˠ);
					ˑ.RemoveQueueItem(H, ˠ);
				}
			}
		}
		void ǭ()
		{
			if (ɽ.Count == 0) return;
			double ƣ = spaceForBottles * 0.12; 
			MyDefinitionId đ = MyItemType.MakeOre("Ice"); 
			string Đ = đ.ToString();
			double ď = 0.00037; 
			foreach (IMyGasGenerator Č in ɽ)
			{
				var Q = Č.GetInventory(0);
				double Ď = G(đ, Č); 
				double Ē = Ď * ď; 
				double č = (double)Q.MaxVolume - ƣ - ď;
				if (Ē > č + ď)
				{
					IMyTerminalBlock Y = e(Č, ʛ);
					if (Y != null) 
					{
						double ú = (Ē - č) / ď; 
						Æ(Đ, Č, 0, Y, 0, ú);
			...
			foreach (var Č in ɽ)
			{
				ċ += G(đ, Č);
				var Q = Č.GetInventory(0); 
				Ċ += (double)Q.MaxVolume;
			}
			double Ĕ = (ċ * ď) / Ċ;
			foreach (var Ĥ in ɽ)
			{
				var h = Ĥ.GetInventory(0); 
				double Ģ = G(đ, Ĥ); 
				double ġ = Ģ * ď;
				double Ġ = (double)h.MaxVolume;
				if (ġ > Ġ * (Ĕ + 0.001))
				{
					foreach (var ğ in ɽ)
					{
						if (Ĥ == ğ) continue; 
						var P = ğ.GetInventory(0); 
						double Ğ = G(đ, ğ);
						double ĝ = Ğ * ď; 
						double Ĝ = (double)P.MaxVolume; 
						if (ĝ < Ĝ * (Ĕ - 0.001))
						{
							double ě = ((Ĝ * Ĕ) - ĝ) / ď;
							if ((Ģ - ě) * ď >= Ġ * Ĕ && ě > 5)
		...
		void ę()
		{
			...
			double Ė = 0;
			double ĕ = 0; 
			foreach (IMyReactor ĉ in ɼ)
			{
				ĉ.UseConveyorSystem = false; 
				double ü = G(Ę, ĉ);
				double ï = uraniumAmountLargeGrid;
				if (ĉ.CubeGrid.GridSize == 0.5f) ï = uraniumAmountSmallGrid; 
				ĕ += ï;
				if (ü > ï + 0.05)
				{
					IMyTerminalBlock Y = e(ĉ, ʚ);
					if (Y != null)
					{
						double ú = ü - ï;
						Æ(ė, ĉ, 0, Y, 0, ú);
			...
			{
				double û = G(Ę, ø);
				double ö = ù * uraniumAmountLargeGrid; 
				if (ø.CubeGrid.GridSize == 0.5f) ö = ù * uraniumAmountSmallGrid;
				if (û > ö + 0.05)
				{
					foreach (var ô in ɼ)
					{
						if (ø == ô) continue;
						double ó = G(Ę, ô); 
						double ò = ù * uraniumAmountLargeGrid; 
						if (ô.CubeGrid.GridSize == 0.5f) ò = ù * uraniumAmountSmallGrid; 
						if (ó < ò - 0.05)
						{
							û = G(Ę, ø); 
							double ñ = ò - ó;
		...
		StringBuilder ð(IMyTextSurface u, bool õ = true, bool ý = true, bool Ă = true, bool Ĉ = true, bool Ć = true)
		{
			bool ą = false; StringBuilder º = new StringBuilder(); if (õ)
			{
				º.Append("Isy's Inventory Manager\n");
				º.Append(u.ō('=', u.ũ(º))).Append("\n\n");
			}
			if (ý && Ŀ != null)
			{
				º.Append("Warning!\n" + Ŀ + "\n\n");
			...
			{
				º.Append(ă(u, ʛ, "Ores")); 
				º.Append(ă(u, ʚ, "Ingots"));
				º.Append(ă(u, ʙ, "Components"));
				º.Append(ă(u, ʘ, "Tools")); 
				º.Append(ă(u, ʗ, "Ammo")); 
				º.Append(ă(u, ʖ, "Bottles"));
				º.Append("=> " + ʉ.Count + " type containers: Balancing " + (balanceTypeContainers ? "ON" : "OFF") + "\n\n");
				ą = true;
			}
			if (Ĉ)
			{
				º.Append("Managed blocks:\n"); 
				float Ą = u.ũ(ɖ.Count.ToString()); 
				º.Append(ɖ.Count + " Inventories (total) / " + ʌ.Count + " have items to sort\n");
				if (ʕ.Count > 0) 
				{
					º.Append(u.ō(' ', Ą - u.ũ(ʕ.Count.ToString())).ToString() + ʕ.Count + " Special Containers\n");
				}
				if (ʅ.Count > 0)
				{
					º.Append(u.ō(' ', Ą - u.ũ(ʅ.Count.ToString())).ToString() + ʅ.Count + " Refineries: ");
					º.Append("Ore Balancing " + (enableOreBalancing ? "ON" : "OFF") + "\n");
				}
				if (ɽ.Count > 0)
				{
				...
				if (ɼ.Count > 0)
				{
					º.Append(u.ō(' ', Ą - u.ũ(ɼ.Count.ToString())).ToString() + ɼ.Count + " Reactors: ");
					º.Append("Uranium Balancing " + (enableUraniumBalancing ? "ON" : "OFF") + "\n");
				}
				if (ʁ.Count > 0
)
				{
					º.Append(u.ō(' ', Ą - u.ũ(ʁ.Count.ToString())).ToString() + ʁ.Count + " Assemblers: ");
					º.Append("Craft " + (enableAutocrafting ? "ON" : "OFF") + " | "); 
					º.Append("Uncraft " + (enableAutodisassembling ? "ON" : "OFF") + " | ");
					º.Append("Cleanup " + ( enableAssemblerCleanup ? "ON" : "OFF") + "\n");
				}
				if (ɾ.Count > 0)
				{
					º.Append(u.ō(' ', Ą - u.ũ(ɾ.Count.ToString())).ToString() + ɾ.Count + " Survival Kits: "); 
					º.Append("Ingot Crafting " + (enableBasicIngotCrafting ? "ON" : "OFF") + (ʅ.Count > 0 ? " (Auto OFF - refineries exist)" : "") + "\n");
				}
				º.Append("\n"); ą = true;
			}
			if (Ć && ɞ != "") 
			{
				º.Append("Last Action:\n" + ɞ);
				ą = true; 
			}
			if (!ą)
			{
				º.Append("-- No informations to show --");
			}
			return º;
		}
		StringBuilder ă(IMyTextSurface u, List<IMyTerminalBlock> ć, string K)
		{
			...
			foreach (var X in ć)
			{
				var Q = X.GetInventory(0);
				ā += (double)Q.CurrentVolume; 
				Ā += (double)Q.MaxVolume;
			}
			string Ê = ć.Count + "x " + K + ":"; 
			string ÿ = ā.Ɓ();
			string ģ = Ā.Ɓ();
			StringBuilder þ = ǝ(u, Ê, ā, Ā, ÿ, ģ);
			return þ;
		}
		void ĥ(string ń = null)
		{
			...
			for (int H = ɦ; H < ɛ.Count; H++)
			{
				if (Ǔ()) return; 
				ɦ++;
				var İ = ɛ[H].ƌ(mainLCDKeyword);
				foreach (var į in İ)
				{
					var ĭ = į.Key; 
					var ħ = į.Value; 
					bool õ = ħ.Ɩ("showHeading");
					bool ý = ħ.Ɩ("showWarnings"); 
					bool Ă = ħ.Ɩ("showContainerStats");
					bool Ĉ = ħ.Ɩ("showManagedBlocks"); 
					bool Ć = ħ.Ɩ("showLastAction");
					bool ł = ħ.Ɩ("scrollTextIfNeeded");
		...
		void Ł()
		{
			...
			StringBuilder ŀ = new StringBuilder();
			if (ɳ.Count == 0)
			{
				ŀ.Append("- No problems detected -");
			}
			else 
			{
				int Ń = 1;
			...
			for (int H = ɥ; H < ɚ.Count; H++)
			{
				if (Ǔ()) return;
				ɥ++; 
				var İ = ɚ[H].ƌ(warningsLCDKeyword); 
				foreach (var į in İ)
				{
					var ĭ = į.Key;
					var ħ = į.Value;
		...
		void ņ()
		{
			...
			for (int H = ɤ; H < ɠ.Count; H++)
			{
				if (Ǔ()) return; 
				ɤ++;
				var İ = ɠ[H].ƌ(performanceLCDKeyword);
				foreach (var į in İ)
				{
					var ĭ = į.Key;
					var ħ = į.Value;
					...
					if (õ)
					{
						º.Append("Isy's Inventory Manager Performance\n");
						º.Append(ĭ.ō('=', ĭ.ũ(º))).Append("\n\n");
					}
					º.Append(ɪ); 
					º = ĭ.Ř(º, õ ? 3 : 0, ł); 
		...
		void Ņ()
		{
			...
			Dictionary<IMyTextSurface, string> Ň = new Dictionary<IMyTextSurface, string>();
			Dictionary<IMyTextSurface, string> ľ = new Dictionary<IMyTextSurface, string>(); 
			List<IMyTextSurface> ļ = new List<IMyTextSurface>();
			List<IMyTextSurface> ı = new List<IMyTextSurface>(); 
			foreach (var W in ə)
			{
				var İ = W.ƌ(inventoryLCDKeyword);
				foreach (var į in İ)
				{
			...
			foreach (var ĭ in Ň)
			{
				Į.Add(System.Text.RegularExpressions.Regex.Match(ĭ.Value, inventoryLCDKeyword + @":[A-Za-z]+").Value); 
			}
			Į.RemoveWhere(Ĳ => Ĳ == ""); 
			List<string> Ĭ = Į.ToList(); 
			for (int H = ɣ; H < Ĭ.Count; H++)
			{
				if (Ǔ()) return;
				ɣ++; 
				var ī = Ň.Where(Ī => Ī.Value.Contains(Ĭ[H]));
				var ĩ = from pair in ī
						orderby System.Text.RegularExpressions.Regex.Match(pair.Value, inventoryLCDKeyword + @":\w+").Value 
						ascending 
						select pair;
				IMyTextSurface Ĩ = ĩ.ElementAt(0).Key;
				string ħ = ĩ.ElementAt(0).Value; 
				StringBuilder º = ķ(Ĩ, ħ);
				if (!ħ.ToLower().Contains("noscroll"))
				{
					int Ħ = 0; 
					foreach (var Ĵ in ĩ)
					{
				...
				foreach (var Ĵ in ĩ)
				{
					IMyTextSurface ĭ = Ĵ.Key;
					ĭ.FontSize = Ĩ.TextureSize.Y / ĭ.TextureSize.Y * Ĩ.FontSize;
					ĭ.Font = Ĩ.Font;
					ĭ.TextPadding = Ĩ.TextPadding;
					ĭ.Alignment = Ĩ.Alignment; 
			...
			for (int H = ɢ; H < ı.Count; H++)
			{
				if (Ǔ()) return;
				ɢ++;
				IMyTextSurface ĭ = ı[H];
				string ħ = ľ[ĭ];
				StringBuilder º = ķ(ĭ, ħ);
				if (!ħ.ToLower().Contains("noscroll"))
				{
					º = ĭ.Ř(º, 0);
				}
		...
		StringBuilder ķ(IMyTextSurface u, string ħ)
		{
			StringBuilder º = new StringBuilder(); 
			var Ķ = ħ.Split('\n').ToList(); 
			Ķ.RemoveAll(ĵ => ĵ.StartsWith("@") || ĵ.Length <= 1);
			bool ĳ = true;
			try
			{
				if (Ķ[0].Length <= 1) ĳ = false;
			}
			catch 
			{
				ĳ = false; 
			...
			else
			{
				foreach (var Ð in Ķ)
				{
					...
					string ª = Ð.ToLower(); 
					if (ª.Contains("noheading")) o = true;
					if (ª.Contains("nobar")) m = true;
					if (ª.Contains("hideempty")) w = true;
					if (ª.Contains("singleline")) k = true; 
					if (ª.StartsWith("echoc"))
					{
						string z = Ð.ŷ("echoc ", "").ŷ("echoc", ""); 
						º.Append(u.ō(' ', (u.ŋ() - u.ũ(z)) / 2)).Append(z + "\n");
					}
					else if (ª.StartsWith("echor"))
					{
						string z = Ð.ŷ("echor ", "").ŷ("echor", "");
						º.Append(u.ō(' ', u.ŋ() - u.ũ(z))).Append(z + "\n");
					}
					else if (ª.StartsWith("echo"))
					{
						º.Append(Ð.ŷ("echo ", "").ŷ("echo", "") + "\n");
					}
					else 
					{
						º.Append(v(u, Ö[0], q, o, m, w, k));
					}
				}
			}
			if (º.Length > 2)
			...
			{
				return new StringBuilder("Nothing to show at the moment...");
			}
		}
		StringBuilder v(IMyTextSurface u, string r, double q, bool o = false, bool m = false, bool w = false, bool k = false)
		{
			StringBuilder º = new StringBuilder();
			bool Ì = q == -1 ? true : false; 
			foreach (var Ë in ȧ)
			{
				if (Ë.ToString().ToLower().Contains(r.ToLower()))
				{
					if (º.Length == 0 && !o)
					{
		...
		{
			ɜ = ɜ >= 3 ? 0 : ɜ + 1;
			Echo("Isy's Inventory Manager " + ɝ[ɜ] + "\n====================\n"); 
			if (Ŀ != null) 
			{
				Echo("Warning!\n" + Ŀ + "\n"); 
			}
			...
			if (ʝ.Count > 0)
			{
				º.Append("[No Sorting] grids:\n==============\n"); 
				foreach (var Ç in ʝ) 
				{
					º.Append(Ç.CustomName + "\n");
				}
				º.Append("\n");
			}
			if (ʜ.Count > 0)
			{
				º.Append("[No IIM] grids:\n===========\n"); 
				foreach (var Ç in ʜ) 
				{
					º.Append(Ç.CustomName + "\n");
				}
		...
		double Æ (string Å, IMyTerminalBlock Ä, int Ã, IMyTerminalBlock Y, int Â, double Á = -1, bool À = false)
		{
			var h = Ä.GetInventory(Ã); 
			var P = Y.GetInventory(Â); 
			if (!h.IsConnectedTo(P))
			{
				ƹ("Item transfer from '" + Ä.CustomName + "'\n" +
					"to '" + Y.CustomName + "'\n" +
					"not possible! There's no valid conveyor path!");
				return 0;
			}
			if (P.IsFull || Á == 0) 
			{
			...
			double N = 0;
			MyDefinitionId M = new MyDefinitionId();
			MyDefinitionId L = new MyDefinitionId();
			string K = "";
			string J = ""; 
			bool I = false;
			string O = "";
			if (Á == -0.5) O = "halfInventory"; 
			if (Á == -1) O = "completeInventory"; 
			for (int H = A.Count - 1; H >= 0; H--)
			{
				M = A[H].Type;
				if (À ? M.ToString() == Å : M.ToString().Contains(Å))
				{
					if (O != "" && M != L) N = 0;
					L = M; 
					K = M.TypeId.ToString().Replace(ȏ, ""); 
					J = M.SubtypeId.ToString();
					I = true;
					if (!h.CanTransferItemTo(P, M))
					{
					...
					double F = (double)A[H].Amount; 
					double E = 0;
					if (O == "completeInventory") 
					{
						h.TransferItemTo(P, H, null, true); 
					}
					else if (O == "halfInventory")
					{
						double D = Math.Ceiling((double)A[H].Amount / 2);
						h.TransferItemTo(P, H, null, true, (VRage.MyFixedPoint)D);
					}
					else 
					{ 
						if (!K.Contains(ȍ)) Á = Math.Ceiling(Á);
						h.TransferItemTo(P, H, null, true, (VRage.MyFixedPoint)Á);
					}
					A.Clear(); 
					h.GetItems(A);
					try
					{
						if ((MyDefinitionId)A[H].Type == M) 
						{
							E = (double)A[H].Amount; 
						}
					}
					...
					Á -= C;
					if (Á <= 0 && O == "") break;
					if ((float) P.CurrentVolume > (float)P.MaxVolume * 0.98) break;
				}
			}
			if (!I) return 0;
			if (N > 0)
			...
			{
				string B = Math.Round(Á, 2) + " " + Å.Replace(ȏ, "");
				if (O == "completeInventory") B = "all items";
				if (O == "halfInventory") B = "half of the items";
				ƹ("Couldn't move '" + B + "'\n" +
					"from '" + Ä.CustomName + "'\n" +
					"to '" + Y.CustomName + "'\n" +
		...
		IMyTerminalBlock d(MyDefinitionId M, bool Z = false, IMyTerminalBlock Y = null)
		{
			try
			{
				if (ʑ.GetInventory(0).FindItem(M) != null && ʑ != Y) 
				{
					return ʑ; 
				}
			}
			...
			foreach (var X in ʌ)
			{
				if (M.SubtypeId.ToString() == "Ice" && X.GetType().ToString().Contains("MyGasGenerator")) continue;
				if (X.GetInventory(0).FindItem(M) != null) 
				{
					ʑ = X;
					return X;
				}
		...
		{
			IMyTerminalBlock V = null; 
			V = U(T, S); 
			if (V != null) return V;
			V = U(T, ʊ);
			if (V == null) ƹ("'" + T.CustomName + "'\nhas no empty containers to move its items!"); 
			return V;
		}
		IMyTerminalBlock U(IMyTerminalBlock T, List<IMyTerminalBlock> S)
		{
			var R = T.GetInventory(0);
			foreach (var X in S)
			{
				if (X == T) continue;
				var Q = X.GetInventory(0);
				if ((float)Q.CurrentVolume < (float)Q.MaxVolume * 0.95f) 
				{
					if (!X.GetInventory(0).IsConnectedTo(R)) continue;
					return X;
				}
			}
			return null;
		...
		{
			string ç = System.Text.RegularExpressions.Regex.Match(W.CustomName, @"\[p(\d+|max|min)\]", System.Text.RegularExpressions.RegexOptions.IgnoreCase).Groups[1].Value.ToLower();
			int æ = 0; 
			bool å = true;
			if (ç == "max")
			{
...
;
			...
			{
				å = Int32.TryParse(ç, out æ);
			}
			if (!å)
			{
				string Ç = W.IsSameConstructAs(Me) ? "" : "1"; 
				Int32.TryParse(Ç + W.EntityId.ToString().Substring(0, 4), out æ);
			}
			return æ;
		}
		string ä(string á)
		...
		{
			foreach (var í in Ǹ.Keys.ToList())
			{
				Ǹ[í] = "0";
			}
			List<string> ì = X.CustomData.Replace(" ", "").TrimEnd('\n').Split('\n').ToList();
			ì.RemoveAll(Ð => !Ð.Contains("=") || Ð.Length < 8); 
			bool ë = false;
			foreach (var Ð in ì)
			{
				var ê = Ð.Split('=');
				if (!Ǹ.ContainsKey(ê[0]))
				{
					MyDefinitionId M; 
					if (MyDefinitionId.TryParse(ȏ + ê[0], out M))
					{
						Ü(M); 
						ë = true; 
					}
			...
			if (ë) Ó(); 
			List<string> é = new List<string>{"Special Container modes:",
				"",
		...
		void Õ()
		{
			for (int H = ɨ; H < ɖ.Count; H++)
			{
				...
				var A = new List<MyInventoryItem>();
				ɖ[H].GetInventory(0).GetItems(A);
				foreach (var Ë in A)
				{
					MyDefinitionId M = Ë.Type; 
					if (ȧ.Contains(M)) continue; 
					ɞ = "Found new item!\n" + 
						M.SubtypeId.ToString() + " (" + M.TypeId.ToString().Replace(ȏ, "") + ")";
					Î(M);
					Ü(M);
					Ư(M);
				}
		...
		{
			string K = M.TypeId.ToString().Replace(ȏ, ""); 
			string J = M.SubtypeId.ToString();
			if (K == Ȏ)
			{
				Ȧ.Add(J);
				Ƣ(J);
		...
		void Ü(MyDefinitionId M)
		{
			...
			foreach (var Ð in Ò)
			{
				try 
				{
					if (Ð.Substring(0, Ð.IndexOf(";")) == M.ToString()) return; 
				} 
				catch { }
			}
			for (int H = Ò.Count - 1; H >= 0; H--) 
			{
				if (Ò[H].Contains(";"))
				{
					Ò.Insert(H + 1, M + ";noBP");
...
;
		...
		void Û() 
		{
			if (!Me.CustomData.Contains(ɫ)) 
			{
				Me.CustomData = (Ƿ ? Ƕ : ǵ) + ɫ; 
			}
		}
		void Ú()
		{
			...
			{
				var A = new List<MyInventoryItem>();
				Ȗ.GetInventory(1).GetItems(A);
				var Ù = new List<MyProductionItem>(); 
				Ȗ.GetQueue(Ù); 
				if (A.Count == 0) return;
				Ȗ.CustomName = ȕ;
				MyDefinitionId Ô = Ù[0].BlueprintId;
				MyDefinitionId M = A[0].Type; 
				if (A.Count == 1 && Ù.Count == 1 && Ȗ.Mode == MyAssemblerMode.Assembly && Ô == Ȕ)
				{
					if (ȕ.Contains(learnKeyword) && !ȕ.Contains(learnManyKeyword)) Ȗ.CustomName = ȕ.Replace(" " + learnKeyword, "").Replace(learnKeyword + " ", "");
					Ȗ.ClearQueue(); 
					Ȕ = new MyDefinitionId(); 
					ɞ = "Learned new Blueprint!\n'" +
						Ô.ToString().Replace(ȏ, "") + "'\n" +
						"produces: '" + M.ToString().Replace(ȏ, "") + "'";
					Ǡ(M); 
					Î(M); 
					ǡ(M, Ô); 
					Ü(M); 
			...
			foreach (var Ø in ɿ)
			{
				var Ù = new List<MyProductionItem>();
				Ø.GetQueue(Ù); 
				if (Ù.Count == 1 && Ø.Mode == MyAssemblerMode.Assembly)
				{
					if (!µ(Ø)) return;
					Ȗ = Ø;
					Ȕ = Ù[0].BlueprintId; 
					ȕ = Ø.CustomName; 
					Ø.CustomName = "Learning " + Ȕ.SubtypeName + " in: " + Ø.CustomName; 
					return;
				}
			}
		}
		...
		bool Ʊ(MyDefinitionId Ô)
		{
			try { 
				foreach (var Ø in ʁ) 
				{
					if (Ø.CanUseBlueprint(Ô)) return true; 
				}
			}
			catch
			{
		...
		{
			if (ʁ.Count == 0) return;
			if (M.TypeId.ToString() == ȏ + Ȏ || M.TypeId.ToString() == ȏ + ȍ) return;
			MyDefinitionId Ô;
			bool ƥ = Ǯ.TryGetValue(M, out Ô);
			if (ƥ) ƥ = Ʊ(Ô); 
			if (!ƥ)
			{
				var ư = new List<string>{"BP","","Component","Magazine","_Blueprint"};
				bool Ʈ = false; 
				foreach (var ƭ in ư)
				{
					string Ƭ = ȩ + M.SubtypeId.ToString().Replace("Item", "") + ƭ;
					MyDefinitionId.TryParse(Ƭ, out Ô);
					Ʈ = Ʊ(Ô); 
					if (Ʈ) 
					{
						ǡ(M, Ô);
		...
		void ƫ(MyDefinitionId M, MyDefinitionId Ô)
		{
			...
			{
				if (Ò[H].Substring(0, Ò[H].IndexOf(";")) != M.ToString()) continue;
				var Ï = Ò[H].Split(';'); 
				Ò[H] = Ï[0] + ";" + Ô.ToString();
				Me.CustomData = String.Join("\n", Ò); 
				return;
		...
		void Ʋ(MyDefinitionId M)
		{
			Û();
			var Ò = Me.CustomData.Split('\n').ToList();
			Ò.RemoveAll(H => H.Contains(M.ToString() + ";"));
			Me.CustomData = String.Join("\n", Ò);
		}
		void ƹ(string ń)
		{
		...
		void Ƹ()
		{
			...
			foreach (var X in ʕ)
			{
				List<string> Ò = X.CustomData.Replace(" ", "").TrimEnd('\n').Split('\n').ToList(); 
				Ò.RemoveAll(Ð => !Ð.Contains("=") || Ð.Contains("=0"));
				X.CustomData = string.Join("\n", Ò);
			}
		...
		{
			Ȝ.Clear(); 
			List<IMyTerminalBlock> ƶ = ʀ.ToList<IMyTerminalBlock>();
			List<IMyTerminalBlock> Ƶ = ʋ.ToList<IMyTerminalBlock>();
			ƴ(ɖ, 0);
			ƴ(ƶ, 1); 
			ƴ(Ƶ, 1);
		}
		void ƴ(List<IMyTerminalBlock> Ƴ, int f)
		{
			...
			{
				var A = new List<MyInventoryItem>();
				Ƴ[H].GetInventory(f).GetItems(A); 
				for (int ƺ = 0; ƺ < A.Count; ƺ++)
				{
					MyDefinitionId M = A[ƺ].Type;
					if (Ȝ.ContainsKey(M))
					{
						Ȝ[M] += (double)A[ƺ].Amount;
					}
					else
					{
						Ȝ[M] = (double)A[ƺ].Amount; 
		...
		double G(MyDefinitionId M)
		{
			double Ź; 
			Ȝ.TryGetValue(M, out Ź);
			return Ź;
		}
		void Ƣ(string Ơ)
		{
		...
		void Ɵ()
		{
			...
			foreach (IMyAssembler Ø in ʁ)
			{
				var Ù = new List<MyProductionItem>();
				Ø.GetQueue(Ù);
				if (Ù.Count > 0 && !Ø.IsProducing)
				{
					if (Ø.Mode == MyAssemblerMode.Assembly) ƹ("'" + Ø.CustomName + "' has a queue but is currently not assembling!\n" +
						"Are there enough ingots for the craft?"); 
					if (Ø.Mode == MyAssemblerMode.Disassembly) ƹ("'" + Ø.CustomName + "' has a queue but is currently not disassembling!\n" +
						"Are the items to disassemble missing?");
				}
				foreach (var Ë in Ù)
				{
					MyDefinitionId Ô = Ë.BlueprintId;
					if (ț.ContainsKey(Ô))
					{
						ț[Ô] += (double)Ë.Amount;
					}
		...
		double ƨ(MyDefinitionId M, bool Ƨ = false) { double Ź; if (!Ȉ.TryGetValue(M, out Ź) && Ƨ) Ź = 10000; return Ź; }
		MyDefinitionId Ʀ(MyDefinitionId M, out bool ƥ) { MyDefinitionId Ô; ƥ = Ǯ.TryGetValue(M, out Ô); return Ô; }
		MyDefinitionId Ƥ(MyDefinitionId Ô) { MyDefinitionId M; Ǻ.TryGetValue(Ô, out M); return M; }
		bool ǋ(MyDefinitionId Ô) { return Ǻ.ContainsKey(Ô); }
		void ǡ(MyDefinitionId M, MyDefinitionId Ô) { Ǯ[M] = Ô; Ǻ[Ô] = M; }
		void Ǡ(MyDefinitionId M) { ȧ.Add(M); ǹ[M.SubtypeId.ToString()] = M; }
		MyDefinitionId ǟ(string J) { MyDefinitionId M = new MyDefinitionId(); ǹ.TryGetValue (J, out M); return M; }
		StringBuilder ǝ(IMyTextSurface u, string Ê, double à, double ǜ, string Ǜ = null, string Ǟ = null, bool m = false, bool Ǣ = false, string Ą = "")
		{
			string ÿ = à.ToString();
			string ģ = ǜ.ToString();
			if (Ǜ != null) 
			...
			char Ǫ = ' '; 
			float ǩ = u.ő(Ǫ); 
			StringBuilder Ǩ = new StringBuilder(" " + à.Ƅ(ǜ));
			Ǩ = u.ō(Ǫ, u.ũ("9999.9%") - u.ũ(Ǩ)).Append(Ǩ); 
			StringBuilder ǧ = new StringBuilder(ÿ + " / " + ģ);
			StringBuilder Ǧ = new StringBuilder(); 
			StringBuilder ǥ = new StringBuilder(); 
			StringBuilder Ǥ;
			if (ǜ == 0)
			{
				Ǧ.Append(Ą + Ê + " "); 
				Ǥ = u.ō(Ǫ, ų - u.ũ(Ǧ) - u.ũ(ÿ));
				Ǧ.Append(Ǥ).Append(ÿ);
				return Ǧ.Append("\n");
			}
			double ǚ = 0;
			if (ǜ > 0) ǚ = à / ǜ >= 1 ? 1 : à / ǜ;
			if (Ǣ && !m)
			{
				if (ū < Ǭ || (ū < ǫ && ų > 512))
				{
					Ǧ.Append(ǣ(u, ų * 0.25f, ǚ, Ą) + " " + Ê + " ");
					Ǥ = u.ō(Ǫ, ų * 0.75 - u.ũ(Ǧ) - u.ũ(ÿ + " /")); 
					Ǧ.Append(Ǥ).Append(ǧ); 
					Ǥ = u.ō(Ǫ, ų - u.ũ(Ǧ) - u.ũ(Ǩ));
					Ǧ.Append(Ǥ);
					Ǧ.Append(Ǩ);
				}
				else
				{
					Ǧ.Append(ǣ(u, ų * 0.3f, ǚ, Ą) + " " + Ê + " "); 
					Ǥ = u.ō(Ǫ, ų - u.ũ(Ǧ) - u.ũ(Ǩ));
					Ǧ.Append(Ǥ);
					Ǧ.Append(Ǩ);
				}
		...
		StringBuilder ǣ(IMyTextSurface u, float Ŭ, double ǚ, string Ą)
		{
			StringBuilder ǉ, ǈ; 
			char Ǉ = '[';
			char ǆ = ']';
			...
			ǉ = u.ō(ǅ, ǀ * ǚ);
			ǈ = u.ō(Ǆ, ǀ - u.ũ(ǉ));
			return new StringBuilder().Append(Ą).Append(Ǉ).Append(ǉ).Append(ǈ).Append(ǆ);
		}
		StringBuilder ƿ = new StringBuilder("No performance Information available!");
		Dictionary<string, int> ƾ = new Dictionary<string, int>();
		List<int> ƽ = new List<int>(new int[600]);
		List<double> Ƽ = new List<double>(new double[600]); 
		double ǂ, ƻ, ǌ, Ǚ, ǘ; 
		int Ǘ, ǖ = 0; 
		void Ǖ(string ǔ)
		{
			ǖ = ǖ >= 599 ? 0 : ǖ + 1;
			Ǘ = Runtime.CurrentInstructionCount;
			if (Ǘ > ƻ) ƻ = Ǘ; 
			ƽ[ǖ] = Ǘ;
			Ǚ = ƽ.Sum() / ƽ.Count;
			ƿ.Clear(); 
			ƿ.Append("Instructions: " + Ǘ + " / " + Runtime.MaxInstructionCount + "\n");
			ƿ.Append("Max. Instructions: " + ƻ + " / " + Runtime.MaxInstructionCount + "\n"); 
			ƿ.Append("Avg. Instructions: " + Math.Floor(Ǚ) + " / " + Runtime.MaxInstructionCount + "\n\n");
			ǂ = Runtime.LastRunTimeMs; 
			if (ǂ > ǌ && ƾ.ContainsKey(ǔ)) ǌ = ǂ; 
			Ƽ[ǖ] = ǂ;
			ǘ = Ƽ.Sum() / Ƽ.Count;
			ƿ.Append("Last runtime: " + Math.Round(ǂ, 4) + " ms\n");
			ƿ.Append("Max. runtime: " + Math.Round(ǌ, 4) + " ms\n");
			ƿ.Append("Avg. runtime: " + Math.Round(ǘ, 4) + " ms\n\n");
			ƿ.Append("Instructions per Method:\n");
			ƾ[ǔ] = Ǘ;
			foreach (var Ë in ƾ.OrderByDescending(H => H.Value))
			{
				ƿ.Append("- " + Ë.Key + ": " + Ë.Value + "\n");
			}
			ƿ.Append("\n");
		}
		bool Ǔ(double à = 10)
		...
		List<IMyTerminalBlock> ǒ(string Ƌ, string[] Ǒ = null, string ǐ = "Debug", float Ǐ = 0.6f, float ǎ = 2f)
		{
			string Ǎ = "[IsyLCD]"; 
			var Ɯ = new List<IMyTerminalBlock>();
			GridTerminalSystem.GetBlocksOfType<IMyTextSurfaceProvider>(Ɯ, ŧ => ŧ.IsSameConstructAs(Me) && (ŧ.CustomName.Contains(Ƌ) || (ŧ.CustomName.Contains(Ǎ) && ŧ.CustomData.Contains(Ƌ))));
			var ƀ = Ɯ.FindAll(ŧ => ŧ.CustomName.Contains(Ƌ)); 
			foreach (var u in ƀ)
			{
				u.CustomName = u.CustomName.Replace(Ƌ, "").Replace(" " + Ƌ, "").TrimEnd(' '); 
				bool Ŧ = false;
				bool ť = false; 
				int Ť = 0; 
				if (u is IMyTextSurface)
				{
					if (!u.CustomName.Contains(Ǎ)) Ŧ = true; 
					if (!u.CustomData.Contains(Ƌ))
					{
						ť = true; 
						u.CustomData = "@0 " + Ƌ + (Ǒ != null ? "\n" + String.Join("\n", Ǒ) : "");
					}
				}
				else if (u is IMyTextSurfaceProvider)
				{
					if (!u.CustomName.Contains(Ǎ)) Ŧ = true;
					int ţ = (u as IMyTextSurfaceProvider).SurfaceCount;
					for (int H = 0; H < ţ; H++)
					{
						if (!u.CustomData.Contains("@" + H))
						{
							ť = true;
							Ť = H; 
							u.CustomData += (u.CustomData == "" ? "" : "\n\n") + "@" + H + " " + Ƌ + (Ǒ != null ? "\n" + String.Join("\n", Ǒ) : ""); 
							break; 
						}
					}
				}
				...
				if (ť)
				{
					var ĭ = (u as IMyTextSurfaceProvider).GetSurface(Ť);
					ĭ.Font = ǐ;
					ĭ.FontSize = Ǐ;
					ĭ.TextPadding = ǎ;
					ĭ.Alignment = TextAlignment.LEFT;
	...
	class Ţ : IComparer<MyDefinitionId>
	{
		...
		{
			return Š.ToString().CompareTo(ş.ToString());
		}
	}
	class š : IEqualityComparer<MyInventoryItem>
	{
		...
		{
			return Š.ToString() == ş.ToString();
		}
		public int GetHashCode(MyInventoryItem Ë) { return Ë.ToString().GetHashCode(); }
	}
	public static partial class Ş
	{
		private static Dictionary<char, float> Ũ = new Dictionary<char, float>();
		public static void Ų(string Ű, float ů) { foreach (char Ū in Ű) { Ũ[Ū] = ů; } }
		public static void Ů()
		{
			if (Ũ.Count > 0) return;
			Ų("3FKTabdeghknopqsuy£µÝàáâãäåèéêëðñòóôõöøùúûüýþÿāăąďđēĕėęěĝğġģĥħĶķńņňŉōŏőśŝşšŢŤŦũūŭůűųŶŷŸșȚЎЗКЛбдекруцяёђћўџ", 18); 
			Ų("ABDNOQRSÀÁÂÃÄÅÐÑÒÓÔÕÖØĂĄĎĐŃŅŇŌŎŐŔŖŘŚŜŞŠȘЅЊЖф□", 22);
			Ų("#0245689CXZ¤¥ÇßĆĈĊČŹŻŽƒЁЌАБВДИЙПРСТУХЬ€", 20);
			Ų("￥$&GHPUVY§ÙÚÛÜÞĀĜĞĠĢĤĦŨŪŬŮŰŲОФЦЪЯжы†‡", 21);
			Ų("！ !I`ijl ¡¨¯´¸ÌÍÎÏìíîïĨĩĪīĮįİıĵĺļľłˆˇ˘˙˚˛˜˝ІЇії‹›∙", 9);
			Ų("？7?Jcz¢¿çćĉċčĴźżžЃЈЧавийнопсъьѓѕќ", 17); 
			Ų("（）：《》，。、；【】(),.1:;[]ft{}·ţťŧț", 10);
			Ų("+<=>E^~¬±¶ÈÉÊË×÷ĒĔĖĘĚЄЏЕНЭ−", 19);
			Ų("L_vx«»ĹĻĽĿŁГгзлхчҐ–•", 16);
			Ų("\"-rª­ºŀŕŗř", 11);
			Ų("WÆŒŴ—…‰", 32);
			Ų("'|¦ˉ‘’‚", 7);
			Ų("@©®мшњ", 26); 
		...
		{
			Ů();
			Vector2 Ŭ = new Vector2(); 
			if (ĭ.Font == "Monospace") 
			{
				float ū = ĭ.FontSize;
				Ŭ.X = (float)(ń.Length * 19.4 * ū); 
				Ŭ.Y = (float)(28.8 * ū);
				return Ŭ; 
			}
			else
			{
				float ū = (float)(ĭ.FontSize * 0.779);
				foreach (char Ū in ń.ToString())
				{
					try 
					{
						Ŭ.X += Ũ[Ū] * ū;
		...
		public static float ũ(this IMyTextSurface u, string ń) { Vector2 ň = u.ŭ(new StringBuilder(ń)); return ň.X; }
		public static float ő(this IMyTextSurface u, char Ő) { float ŏ = ũ(u, new string(Ő, 1)); return ŏ; }
		public static int Ŏ(this IMyTextSurface u) 
		{ 
			Vector2 Ŋ = u.SurfaceSize;
			float ŉ = u.TextureSize.Y; 
			if (Ŋ.X < 512 || ŉ != Ŋ.Y) Ŋ.Y *= 512 / ŉ;
			float Ō = Ŋ.Y * (100 - u.TextPadding * 2) / 100; 
			Vector2 ň = u.ŭ(new StringBuilder("T")); 
			return (int)(Ō / ň.Y);
		}
		public static float ŋ(this IMyTextSurface u)
		{
			Vector2 Ŋ = u.SurfaceSize;
			float ŉ = u.TextureSize.Y; 
			if (Ŋ.X < 512 || ŉ != Ŋ.Y) Ŋ.X *= 512 / ŉ; 
			return Ŋ.X * (100 - u.TextPadding * 2) / 100;
		}
		public static StringBuilder ō(this IMyTextSurface u, char ŗ, double ŝ)
		{
			int ś = (int)(ŝ / ő(u, ŗ));
			if (ś < 0) ś = 0;
			return new StringBuilder().Append(ŗ, ś);
		}
		private static DateTime Ś = DateTime.Now;
		private static Dictionary<int, List<int>> ř = new Dictionary<int, List<int>>();
		public static StringBuilder Ř(this IMyTextSurface u, StringBuilder ń, int Ŝ = 3, bool ł = true, int Ĺ = 0)
		{
			int Ŗ = u.GetHashCode(); 
			if (!ř.ContainsKey(Ŗ))
			{
			...
			int œ = ř[Ŗ][2]; 
			int Œ = ř[Ŗ][3]; 
			var ű = ń.ToString().TrimEnd('\n').Split('\n'); 
			List<string> Ľ = new List<string>();
			if (Ĺ == 0) Ĺ = u.Ŏ();
			float ų = u.ŋ();
			StringBuilder Ą, Ö = new StringBuilder();
			for (int H = 0; H < ű.Length; H++)
			{
				if (H < Ŝ || H < œ || Ľ.Count - œ > Ĺ || u.ũ(ű[H]) <= ų)
				{
					Ľ.Add(ű[H]); 
				}
				else
				{
					try
					{
						...
						float ƒ, Ƒ;
						var Ɛ = ű[H].Split(' ');
						string Ə = System.Text.RegularExpressions.Regex.Match(ű[H], @"\d+(\.|\:)\ ").Value; 
						Ą = u.ō(' ', u.ũ(Ə));
						foreach (var Ɠ in Ɛ)
						{
							ƒ = u.ũ(Ö);
			...
			if (ł)
			{
				if (Ľ.Count > Ĺ)
				{
					...
					{
						Œ = DateTime.Now.Second;
						if (Ŕ > 0) Ŕ--; 
						if (Ŕ <= 0) œ += ŕ;
						if (œ + Ĺ - Ŝ >= Ľ.Count && Ŕ <= 0)
						{
							ŕ = -1; 
							Ŕ = 3; 
						}
			...
			StringBuilder Ǝ = new StringBuilder();
			for (var Ð = 0; Ð < Ŝ; Ð++) { Ǝ.Append(Ľ[Ð] + "\n"); }
			for (var Ð = œ; Ð < Ľ.Count; Ð++) { Ǝ.Append(Ľ[Ð] + "\n"); }
			return Ǝ;
		}
		public static Dictionary<IMyTextSurface, string> ƌ(this IMyTerminalBlock W, string Ƌ, Dictionary<string, string> Ɗ = null)
		{
			var ƍ = new Dictionary<IMyTextSurface, string>(); 
			if (W is IMyTextSurface)
			{
				ƍ[W as IMyTextSurface] = W.CustomData; 
			}
			else if (W is IMyTextSurfaceProvider)
			{
				var Ɖ = System.Text.RegularExpressions.Regex.Matches(W.CustomData, @"@(\d).*(" + Ƌ + @")");
				int Ɣ = (W as IMyTextSurfaceProvider).SurfaceCount;
				foreach (System.Text.RegularExpressions.Match ƛ in Ɖ)
				{
					int ƙ = -1;
					if (int.TryParse(ƛ.Groups[1].Value, out ƙ))
					{
						if (ƙ >= Ɣ) continue; 
						string Ò = W.CustomData;
						int Ƙ = Ò.IndexOf("@" + ƙ); 
						int Ɨ = Ò.IndexOf("@", Ƙ + 1) - Ƙ;
						string ħ = Ɨ <= 0 ? Ò.Substring(Ƙ) : Ò.Substring(Ƙ, Ɨ);
						ƍ[(W as IMyTextSurfaceProvider).GetSurface(ƙ)] = ħ;
					}
				}
			}
			return ƍ;
			...
			{ if (Ð.StartsWith(á + "="))
				{
					try 
					{
						return Convert.ToBoolean(Ð.Replace(á + "=", ""));
					} 
					catch 
					{
						return true; 
		...
		public static string ƕ(this string ħ, string á)
		{
			var Ò = ħ.Replace(" ", "").Split('\n');
			foreach (var Ð in Ò)
			{
	...
	public static partial class Ş
	{
		public static bool ƚ(this double à, double ƈ, double ģ, bool ž = false, bool Ž = false)
		{
			bool ż = à >= ƈ;
			bool Ż = à <= ģ; 
			if (Ž) ż = à > ƈ;
	...
	public static partial class Ş
	{
		public static string ſ(this char ź, int Ÿ)
		{ if (Ÿ <= 0) { return ""; } return new string(ź, Ÿ); }
	}
	public static partial class Ş
	{
		public static string ŷ(this string Ŷ, string ŵ, string Ŵ)
		{
			string Ź = System.Text.RegularExpressions.Regex.Replace(Ŷ, System.Text.RegularExpressions.Regex.Escape(ŵ), Ŵ, System.Text.RegularExpressions.RegexOptions.IgnoreCase);
			return Ź;
		}
	}
	public static partial class Ş
	{
		public static string Ɓ(this float à)
		{
			...
			else if (à >= 1000000 && à < 1000000000) { à /= 1000000; ƅ = "BL"; }
	...
	public static partial class Ş
	{
		public static string Ɔ(this double à)
		{
			string ƅ = ""; if (à >= 1000 && à < 1000000) { à /= 1000; ƅ = " k"; }
			else if (à >= 1000000 && à < 1000000000) { à /= 1000000; ƅ = " M"; }
			else if (à >= 1000000000) { à /= 1000000000; ƅ = " B"; }
			return Math.Round(à, 1) + ƅ;
		}
	}
	public static partial class Ş
	{
		public static string Ƅ(this double ƃ, double ē)
		{ 
			double Ƃ = Math.Round(ƃ / ē * 100, 1);
			if (ē == 0)
			{
				return "0%";
			}
		...
		public static string Ƅ(this float ƃ, float ē)
		{
			double Ƃ = Math.Round(ƃ / ē * 100, 1); 
			if (ē == 0) 
			{
