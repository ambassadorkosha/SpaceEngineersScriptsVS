...
{
    [MyTextSurfaceScript("DailyEffects", "Daily Effects")]
    class DailyEffectsLCDScript : MyTSSCommon
    {
        private IMyCubeBlock block;
        public DailyEffectsLCDScript(IMyTextSurface surface, IMyCubeBlock block, Vector2 size) : base(surface, block, size) { this.block = block; }
        public override void Run()
        {
            ...
            using (var frame = m_surface.DrawFrame())
            {
                try
                {
                    var BlackBox = MySprite.CreateSprite("Circle", Vector2.Zero, m_surface.SurfaceSize * 2);
                    BlackBox.Position = new Vector2(m_surface.SurfaceSize.X / 2, m_surface.SurfaceSize.Y / 2);
                    BlackBox.Color = Color.Black;
                    frame.Add(BlackBox);
                    var Text = MySprite.CreateText(DailyEffects.Effects.ToString(), "Monospace", m_foregroundColor, m_fontScale / 2, TextAlignment.LEFT);
    ...
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_MyProgrammableBlock), true)]
    class DailyChanges : MyGameLogicComponent
    {
        public override void Init(MyObjectBuilder_EntityBase objectBuilder)
        {
            var changesMiningProperty = MyAPIGateway.TerminalControls.CreateProperty<SortedDictionary<float, HashSet<string>>, IMyProgrammableBlock>("changesMining");
            changesMiningProperty.Getter = (b) => DailyEffects.changesAssembler;
            MyAPIGateway.TerminalControls.AddControl<IMyProgrammableBlock>(changesMiningProperty);
            var changesRefineryProperty = MyAPIGateway.TerminalControls.CreateProperty<SortedDictionary<float, HashSet<string>>, IMyProgrammableBlock>("changesRefinery");
            changesRefineryProperty.Getter = (b) => DailyEffects.changesRefinery;
            MyAPIGateway.TerminalControls.AddControl<IMyProgrammableBlock>(changesRefineryProperty);
            var changesAssemblerProperty = MyAPIGateway.TerminalControls.CreateProperty<SortedDictionary<float, HashSet<string>>, IMyProgrammableBlock>("changesAssembler");
            changesAssemblerProperty.Getter = (b) => DailyEffects.changesAssembler;
            MyAPIGateway.TerminalControls.AddControl<IMyProgrammableBlock>(changesAssemblerProperty);
        }
    }
    ...
    public class DailyEffects : MySessionComponentBase
    {
        ...
        public static SortedDictionary<float, HashSet<string>> changesRefinery, changesAssembler, changesMining;

        public static StringBuilder Effects = new StringBuilder();
        ...
        public override void Init(MyObjectBuilder_SessionComponent sessionComponent)
        {
            ...
            if (MyAPIGateway.Session.IsServer)
            {
                ...
                MyAPIGateway.Multiplayer.RegisterSecureMessageHandler(43501, OnRequestMessage);
            }
            else
            {
                MyAPIGateway.Multiplayer.RegisterSecureMessageHandler(43500, OnMessageFromServer);
            }
        ...
        protected override void UnloadData()
        {
            ...
            if (!MyAPIGateway.Session.IsServer)
            {
                MyAPIGateway.Multiplayer.UnregisterSecureMessageHandler(43500, OnMessageFromServer);
            }
            else
            {
                MyAPIGateway.Multiplayer.UnregisterSecureMessageHandler(43501, OnRequestMessage);
            }
        }

        private void Utilities_MessageEntered(string messageText, ref bool sendToOthers)
        {
            if (messageText.ToLower().StartsWith("!daily"))
            {
                sendToOthers = false;
                if (difference == null)
                {
        ...
        public void UpdateSeed()
        {
            ...
            if (newvalue != seed)
            {
                MyVisualScriptLogicProvider.SendChatMessage("Seed:" + newvalue, "Economy system");
                seed = newvalue;
                ...
                {
                    MyVisualScriptLogicProvider.SendChatMessage(e.ToString(), "Economy system");
                }
        ...
        public void Request()
        {
            if (MyAPIGateway.Multiplayer != null)
            {
                try
                {
                    if (!requested)
                    {
                        MyAPIGateway.Multiplayer.SendMessageToServer(43501, new byte[0]);
                        requested = true;
                    }
                }
                catch (Exception e) { }
        ...
        public static void OnRequestMessage(ushort HandlerId, byte[] data, ulong SteamUserId, bool fromServer)
        {
            try
            {
                MyAPIGateway.Multiplayer.SendMessageTo(43500, cached_response_data, SteamUserId);
            }
            catch (Exception e) { }
        }
        ...
        public static void OnMessageFromServer(ushort HandlerId, byte[] data, ulong SteamUserId, bool fromServer)
        {
            try
            {
        ...
        {
            changesRefinery = new SortedDictionary<float, HashSet<string>>();
            changesAssembler = new SortedDictionary<float, HashSet<string>>();
            changesMining = new SortedDictionary<float, HashSet<string>>();
            if (diff.blueprints.Count == 0 && diff.voxels.Count == 0)
            {
                MyVisualScriptLogicProvider.SendChatMessage("All values were set to default", "Economy", MyAPIGateway.Session.Player?.IdentityId ?? 0);
                return;
            }
            ...
            if (diff.voxels.Count != 0)
            {
                ...
                {
                    HashSet<string> set;
                    if (!changesMining.ContainsKey(x.Value))
                    {
                        set = new HashSet<string>();
                        changesMining.Add(x.Value, set);
                    }
                    else
                    {
                        set = changesMining[x.Value];
                    }
                ...
                foreach (var x in changesMining)
                {
                ...
                Effects.Append("Economy: " + sb + "\n");
                MyVisualScriptLogicProvider.SendChatMessage(sb.ToString(), "Economy", MyAPIGateway.Session.Player?.IdentityId ?? 0);
            }
